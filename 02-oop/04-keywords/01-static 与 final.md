...
> **类比**：static 变量就像**公司共享打印机**——不属于任何个人工位，而是公司的公共资源，所有员工（对象）共用一台。实例变量则是每人桌上的**私人文具**，各用各的。
...
> **类比**：final 变量就像**出生证明**——一旦签发，信息不能修改。但出生证明上写的家庭地址（引用）可以搬家，只是证明本身不能换。
...
**Q：static 代码块和字段初始化的执行顺序？**
A：按代码书写顺序执行。`static int a = 1; static { a = 2; }` → a 最终为 2。如果有继承，父类 static 先于子类 static。
