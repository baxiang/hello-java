...
| Spring Boot Extension Pack | Spring Boot 开发支持（含 Boot Dashboard、属性补全、Bean 导航） |
| Gradle for Java | Gradle 构建支持 |
| Lombok Annotations Support | Lombok 注解支持 |

> **Spring Boot Extension Pack** 包含：
> - Spring Boot Dashboard：可视化管理 Spring Boot 应用
> - Spring Initializr：在 VSCode 中创建 Spring Boot 项目
> - Spring Boot Tools：属性补全、Bean 导航、Live Information
...
> **Bracket Pair Colorizer 已废弃**：VSCode 已内置括号配对着色，在 settings.json 中设置 `editor.bracketPairColorization.enabled: true` 即可。
...
| Cline | AI 编程助手，支持自主编写代码（可接入多种模型） |
| Continue | 开源 AI 编程助手，支持本地模型 |
...
| 数据库工具 | 需要扩展 | 内置 |
| 价格 | 免费 | Community 免费 / Ultimate 付费 |
